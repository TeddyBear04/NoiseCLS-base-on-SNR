...
The repository implements a 36-label single-label noise-only BEATs fine-tuning pipeline. Its primary entrypoint is `main.py`; source is organized in `config/`, `models/`, `noise_pipeline/`, `tasks/`, and `utils/`. Edit `config/train_config.json` to control training defaults. The dataset defaults to `../../36_labels/`, with `manifest.csv`, `labels.txt`, and WAV files referenced by `noise_path`; pretrained and fine-tuned weights live in `checkpoint/`; generated embedding caches live in `artifacts/`.
