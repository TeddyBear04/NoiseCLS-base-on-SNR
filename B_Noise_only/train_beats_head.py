"""Train a 36-class head on frozen BEATs embeddings from oracle noise."""
...
def cache_name(split: str, per_class: int | None) -> str:
    ...
    return f"beats_{INPUT_KIND}_{split}_{suffix}_seed2026.pt"
...
def load_or_extract(
    ...
    dataset = NoiseOnlyDataset(root, split, rows=selected)
...
def main() -> None:
    ...
    parser.add_argument("--output", type=Path, default=Path("checkpoint/beats_head_36_noise.pt"))
    parser.add_argument(
        "--results", type=Path,
        default=Path("checkpoint/beats_head_36_noise_summary.json"),
    )
    ...
    if args.smoke_test:
        ...
        args.epochs = 2
        args.workers = 0
        args.output = Path("checkpoint/beats_head_36_noise_smoke.pt")
        args.results = Path("checkpoint/beats_head_36_noise_smoke.json")
    ...
    result = {
        "stage": "frozen_beats_noise_only_head",
