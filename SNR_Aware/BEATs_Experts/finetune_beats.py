"""Fine-tune the final BEATs transformer blocks on mixture waveforms."""
...
class MixtureDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        row = self.rows[index]
        return {
            "mixture": load_float_audio(
                self.root / row["mixture_path"], 16_000, 4 * 16_000
            ),
...
def load_model(
    ...
    pretrained_checkpoint: Path = BEATS_CHECKPOINT,
):
    BEATs, BEATsConfig = load_beats_classes()

    source = torch.load(pretrained_checkpoint, map_location="cpu", weights_only=True)
    ...
    config = model.cfg
    print(
        f"encoder_layers={len(model.encoder.layers)} "
        f"trainable_blocks={trainable_blocks} "
        f"layer_wise_gradient_decay_ratio="
        f"{getattr(config, 'layer_wise_gradient_decay_ratio', 1.0)} "
        f"encoder_layerdrop={config.encoder_layerdrop} "
        f"deep_norm={config.deep_norm}",
        flush=True,
    )
...
def block_gradient_norms(model) -> dict[int, float]:
    """Gradient magnitude per encoder block, to expose layer-wise decay."""
    ...
    for index, layer in enumerate(model.encoder.layers):
        ...
        if total:
            norms[index] = total**0.5
    return norms


def block_weight_deltas(model, baseline: dict[str, torch.Tensor]) -> dict[int, float]:
    """How far each encoder block moved away from the pretrained weights."""
    deltas: dict[int, float] = {}
    current = model.state_dict()
    for key, value in baseline.items():
        index = int(key.split(".")[2])
        difference = current[key].detach().cpu().float() - value.float()
        deltas[index] = deltas.get(index, 0.0) + difference.pow(2).sum().item()
    return {index: total**0.5 for index, total in sorted(deltas.items())}


def format_block_values(values: dict[int, float]) -> str:
    return " ".join(f"{index}:{value:.3e}" for index, value in sorted(values.items()))
...
def train_epoch(
    ...
    for batch_index, batch in enumerate(loader, start=1):
        ...
        if batch_index == accumulation_steps:
            print(
                f"block_grad_norms {format_block_values(block_gradient_norms(model))}",
                flush=True,
            )
...
def main() -> None:
    ...
    parser.add_argument(
        "--head-checkpoint", type=Path, default=Path("checkpoints/beats_head.pt")
    )
    parser.add_argument(
        "--output", type=Path, default=Path("checkpoints/beats_last2.pt")
    )
    parser.add_argument(
        "--results", type=Path,
        default=Path("benchmark_results/beats_last2_full.json"),
    )
    ...
    if args.smoke_test:
        ...
        args.workers = 0
        args.output = Path("checkpoints/beats_last2_smoke.pt")
        args.results = Path("benchmark_results/beats_last2_smoke.json")
    ...
    train_dataset = MixtureDataset(args.data_root, "train", train_rows)
    validation_dataset = MixtureDataset(
        args.data_root, "validation", validation_rows
    )
    model, head, labels = load_model(
        device, args.head_checkpoint, args.trainable_blocks, args.pretrained_checkpoint
    )
    ...
    pretrained_encoder = trainable_encoder_state(model, args.trainable_blocks)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            "block_weight_deltas "
            f"{format_block_values(block_weight_deltas(model, pretrained_encoder))}",
            flush=True,
        )
        print(
    ...
    torch.save(
        {
            ...
            "head": best_head,
            "labels": labels,
            "base_encoder_checkpoint": str(args.pretrained_checkpoint),
