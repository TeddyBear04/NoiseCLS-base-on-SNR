...
def filter_snr_rows(
    rows: list[dict[str, str]], snr_min_db: float | None, snr_max_db: float | None
) -> list[dict[str, str]]:
    """Keep manifest examples in the selected expert's inclusive SNR interval."""
    selected = [
        row
        for row in rows
        if (snr_min_db is None or float(row["target_snr_db"]) >= snr_min_db)
        and (snr_max_db is None or float(row["target_snr_db"]) <= snr_max_db)
    ]
    if not selected:
        raise ValueError(
            f"No manifest rows in SNR interval [{snr_min_db}, {snr_max_db}]."
        )
    return selected
...
def balanced_rows(
    ...
    active_snrs = sorted({snr for _, snr in groups})
    if per_class % len(active_snrs):
        raise ValueError(f"per_class must be divisible by {len(active_snrs)}")
    per_group = per_class // len(active_snrs)
...
def main() -> None:
    ...
    print(
        f"device={device} labels={len(labels)} snr_range="
        f"[{args.snr_min_db}, {args.snr_max_db}] rows={len(rows)}",
        flush=True,
    )
    model, checkpoint = load_beats(device, args.pretrained_checkpoint)
